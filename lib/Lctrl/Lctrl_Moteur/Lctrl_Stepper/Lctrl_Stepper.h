...
public:
    ...
    LctrlStepper(unsigned char pinStep, unsigned char pinDir = -1, unsigned char pinEnable = -1, unsigned short stepsRevolution = 200, unsigned short microstep = 1, unsigned short vMax = 1000, unsigned char mode = Mode_auto,  unsigned char csgMin = 0, unsigned char csgMax = 0, unsigned char rampeAcc = 0);
    void setup(void) override;
    void main(void) override;
    
    void setSpeed(unsigned short vitesse) {m_vitesse = vitesse;}
    void step(int nStep = 1) {m_step = nStep * (unsigned long) m_microStep; m_csgActuelle = 1; m_KmAr = m_step < 0;}
